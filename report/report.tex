...
This architecture was mainly tested in the single agent navigation round, since it didn't seem strong enough to generalize well in a multi-agent environment: in-depth results are shown in section \ref{sec:entire-graph-res} of the next chapter.
...
\begin{table}[h]
	...
	\begin{tabular}{@{}lm{5cm}@{}}
		...
		\texttt{model.hidden\_sizes} & Number and size of FC layers \\ 
		\texttt{model.nonlinearity} & Activation function (Tanh or ReLU) \\ 
		\texttt{model.double} & Enable or disable double DQN \\ 
		\texttt{model.dueling} & Enable or disable dueling DQN  \\  
		...
		\texttt{parameter_decay.type} & Exploration type (Linear or Exponential) \\
		\texttt{replay\_buffer.size} & Replay buffer maximum size \\ 
		\texttt{replay\_buffer.batch\_size} & Size of a batch of experiences \\
		\texttt{replay\_buffer.checkpoint} & How often to learn \\ \bottomrule
	\end{tabular}
...
Table \ref{table:params-desc} contains a high-level description of all the required hyperparameters, while table \ref{table:common-params} shows some hyperparameters that are common to all the experiments shown in subsequent sections.
...
\begin{table}[h]
	...
	\begin{tabular}{@{}lc@{}}
		...
		\texttt{model.double}       &  True    \\ 
		\texttt{model.dueling}       &  True    \\  
...
\begin{table}[h]
	...
	\begin{tabular}{@{}lc@{}}
		...
		\texttt{model.hidden\_sizes}       &  $[128,128]$    \\ 
		\texttt{model.nonlinearity}       &  Tanh    \\ 
...
\begin{table}[h]
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		...
		\textbf{Deadlocks}  & $0.0307$ & $0.0580$ & $0.0596$ & $0.1340$ & $0.1023$ & $0.1254$ \\ \hline
...
\begin{table}[h]
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		\hline
							   ...
							   & \textbf{A} & \textbf{B} & \textbf{A} & \textbf{B} & \textbf{A} & \textbf{B} \\ \hline
		\textbf{Dones}      & $86.28\%$ & & $84.17\%$ & & $76.90\%$ & \\ \hline
		\textbf{Deadlocks}  & $0.0324$ & & $0.0589$ & & $0.1254$ & \\ \hline
		\textbf{Return} & $-0.2331$ &  & $-0.2449$ &  & $-0.2790$ &  \\ \hline
		\textbf{Steps}   & $400$ &  & $447$ &  & $497$ & \\ \hline
	\end{tabular}
...
\section*{Entire graph embeddings}\label{sec:entire-graph-res}
\subsection*{Single agent entire graph embeddings vs. random baseline}\label{sec:entire-graph-single-agent-res}

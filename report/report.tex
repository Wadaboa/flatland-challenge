...
\usepackage{tikz}
\usetikzlibrary{arrows.meta,shadows,positioning,shapes.geometric}
\usepackage{amssymb}
...
Again, in-depth results are shown in section \ref{sec:fov-dqn-gnn-robot-res}.
...
\begin{longtable}{@{}m{5cm}m{5cm}@{}}
	...
	\texttt{training.train_env.episodes} &   Number of training episodes    \\
	\texttt{env.seed} &   Random seed to generate new railways during training    \\
	\texttt{env.num\_trains} &   Number of agents    \\
	\texttt{env.rewards.stop\_penalty} &  Stop penalty coefficient (see \ref{sec:rewards-shaping})    \\
	\texttt{predictor.max\_depth} &  Maximum predictor depth   \\
	\texttt{observator.max\_depth} & Maximum observator depth    \\
	\texttt{observator.normalization\_radius} & Maximum distance to consider in feature normalization \\
	\texttt{learning.learning\_rate} & Learning rate for the Adam optimizer \\
	\texttt{learning.tau} & DQN soft update weight \\
	\texttt{learning.discount} & DQN discount factor \\ 
	\texttt{learning.softmax\_bellman.enabled} & Enable or disable softmax Bellman \\  
	\texttt{learning.softmax\_bellman.temperature} & $\tau$ parameter in softmax Bellman (see \ref{subsec:bellman-operators}) \\ 
	\texttt{learning.loss} & Regression loss to use (Huber or MSE) \\
	\texttt{action_selector.type} & Action selector to use ($\epsilon$-greedy, Random, Greedy, Boltzmann or Categorical) \\
	\texttt{parameter_decay.decaying_episodes} & Percentage of episodes in which the exploration parameter should reach its minimum \\
	\texttt{parameter_decay.start} & Initial exploration value \\
	\texttt{parameter_decay.end} & Final exploration value \\
	\texttt{parameter_decay.type} & Exploration decay type (Linear or Exponential) \\
	\texttt{replay\_buffer.size} & Replay buffer maximum size \\ 
	\texttt{replay\_buffer.batch\_size} & Size of a batch of experiences \\
	\texttt{replay\_buffer.checkpoint} & How often to learn \\ 
	\rowcolor{red!50}\multicolumn{2}{@{}c@{}}{\textbf{DQN}} \\
	\texttt{model.dqn.hidden\_sizes} & Number and size of FC layers \\ 
	\texttt{model.dqn.nonlinearity} & Activation function (Tanh or ReLU) \\ 
	\texttt{model.dqn.double} & Enable or disable double DQN \\ 
	\texttt{model.dqn.dueling.enabled} & Enable or disable dueling DQN  \\
	\texttt{model.dqn.dueling.aggregation} & Advantage function aggregation in dueling DQN (Max or Mean)  \\
	\rowcolor{red!50}\multicolumn{2}{@{}c@{}}{\textbf{Entire graph embeddings}} \\
	\texttt{model.entire_gnn.embedding_size} & Size of the output GNN embeddings \\
	\texttt{model.entire_gnn.hidden_size} & Hidden embedding size of GNN layers  \\
	\texttt{model.entire_gnn.pos_size} & Number of graph embeddings to use as DQN input \\
	\texttt{model.entire_gnn.dropout} & Probability of dropout \\
	\texttt{model.entire_gnn.nonlinearity} & Graph convolutions activation function (Tanh or ReLU) \\
	\rowcolor{red!50}\multicolumn{2}{@{}c@{}}{\textbf{Multi-agent graph embeddings}} \\
	\texttt{model.multi_gnn.cnn_encoder.conv.kernel_size} & Kernel size of convolutions in the CNN encoder \\
	\texttt{model.multi_gnn.cnn_encoder.conv.stride} & Stride of convolutions in the CNN encoder \\
	\texttt{model.multi_gnn.cnn_encoder.conv.padding} & Padding of convolutions in the CNN encoder \\
	\texttt{model.multi_gnn.cnn_encoder.pool.kernel_size} & Kernel size of pooling in the CNN encoder \\
	\texttt{model.multi_gnn.cnn_encoder.pool.stride} & Stride of pooling in the CNN encoder \\
	\texttt{model.multi_gnn.cnn_encoder.pool.padding} & Padding of pooling in the CNN encoder \\
	\texttt{model.multi_gnn.cnn_encoder.hidden_channels} & Number and sizes of hidden channels in the CNN encoder \\
	\texttt{model.multi_gnn.cnn_encoder.output_channels} & Number of output channels in the CNN encoder \\
	\texttt{model.multi_gnn.mlp_compression.hidden_sizes} & Number and sizes of FC layers in the MLP compressor \\
	\texttt{model.multi_gnn.mlp_compression.output_size} & Output size of FC layers in the MLP compressor \\
	\texttt{model.multi_gnn.gnn_communication.hidden_sizes} & Number and sizes of GNN layers in the communicator\\
	\texttt{model.multi_gnn.gnn_communication.embedding_size} & Final embedding size in the GNN communicator \\
	\texttt{model.multi_gnn.gnn_communication.dropout} & Probability of dropout in the GNN layers \\
	\texttt{model.multi_gnn.nonlinearity} & Activation function (Tanh or ReLU) \\
	\bottomrule
...
Table \ref{table:params-desc} contains a high-level description of all the required hyperparameters, while table \ref{table:common-params} shows some hyperparameters that are common to all the experiments presented in subsequent sections.
...
\begin{table}[h]
	...
	\begin{tabular}{@{}lc@{}}
		...
		\texttt{learning.learning\_rate}                 &   $0.5e-4$   \\
		\texttt{learning.tau}                  &   $1e-3$    \\
		\texttt{learning.discount}       &  $0.99$     \\ 
		\texttt{learning.softmax\_bellman.enabled}       &  True     \\  
		\texttt{model.dqn.double}       &  True    \\ 
		\texttt{model.dqn.dueling.enabled}       &  True    \\  
		\texttt{replay\_buffer.size}       &  $\num{100000}$    \\ 
		\texttt{replay\_buffer.batch\_size}       &  $128$    \\
		\texttt{replay\_buffer.checkpoint}       &  $4$    \\ \bottomrule
	\end{tabular}
	\caption{Common hyperparameters}
	\label{table:common-params}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabular}{@{}lc@{}}
		...
		\texttt{env.rewards.stop\_penalty} &   $2.0$    \\
		\texttt{predictor.max\_depth}                 &   $6$   \\
		\texttt{observator.max\_depth}                  &   $6$    \\
		\texttt{observator.normalization\_radius}       &  $30$     \\ 
		\texttt{learning.softmax\_bellman.temperature}       &  $1.0$     \\ 
		\texttt{learning.loss}       &  Huber     \\ 
		\texttt{model.dqn.hidden\_sizes}       &  $[128,128]$    \\ 
		\texttt{model.dqn.nonlinearity}       &  Tanh    \\ 
		\texttt{action_selector.type} & $\epsilon$-greedy \\
		\texttt{parameter_decay.decaying_episodes} & $0.7$ \\
		\texttt{parameter_decay.start} & $1.0$ \\
		\texttt{parameter_decay.end} & $0.01$ \\
		\texttt{parameter_decay.type} & Linear \\
		\bottomrule
	\end{tabular}
...
\begin{table}[h]
	...
	\begin{tabular}{@{}lc@{}}
		...
		\texttt{observator.normalization\_radius}       &  $30$     \\ 
		\texttt{learning.softmax\_bellman.temperature}       &  $0.5$     \\ 
		\texttt{learning.loss}       &  Huber     \\ 
		\texttt{model.dqn.hidden\_sizes}       &  $[128,128]$    \\ 
		\texttt{model.dqn.nonlinearity}       &  Tanh    \\ 
		\texttt{action_selector.type} & $\epsilon$-greedy \\
...
\begin{table}[h]
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		...
		\textbf{Deadlocks}  & $0.0640$ & $0.0680$ \\ \hline
...
\section*{Boltzmann vs. $\epsilon$-greedy action selector}\label{sec:action-selectors-res}
This experiment aims to compare the performance of two action selectors, namely the standard $\epsilon$-greedy and the Boltzmann ones. 
...
Figure \ref{fig:boltzmann-epsgreedy-metrics} shows training metrics comparing \texttt{bt-softmaxbellman} (orange in the plots), which is the DQN model using the binary tree observator, the softmax Bellman operator and an $\epsilon$-greedy selection strategy, and \texttt{bt-softmax\-bellman-boltzmann} (light orange in the plots), which is the same model as the one above, with the excpetion of leveraging the Boltzmann action selector. Both exploration parameters ($\epsilon$ and the temperature $\tau$) followed the same starting/ending values and decay, i.e. a linear one over a pre-defined percentage of training episodes.

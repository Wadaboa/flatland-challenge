...
The new action space is composed by the following choices:
\begin{enumerate}
	\item \texttt{CHOICE_LEFT}: 
	\item \texttt{CHOICE_RIGHT}: 
	\item \texttt{STOP}: as with the standard \texttt{STOP_MOVING} action, the agent decides to stand still (one-to-one mapping)
\end{enumerate}

\section{Observations}
An observation is a representation of the environment in a particular state. Ideally, this snapshot should be rich enough to convey the right amount of information (so as to distinguish between different configurations), but also small enough that it would be efficient to compute it at any given time-step. An observation should also be able to integrate information from local (in our case a train) and global (in our case the grid and all the agents) perspectives.
...
\end{itemize}
Here, the parameters $v_w$ and $v_h$ define the rectangular view of the agent (of dimension $v_w\times v_h$) around its position.
...
One key issue related to the described tree observation is related to efficiency. In particular, we mentioned a maximum depth parameter that controls how far the observation should reach, in terms of consecutive switches, targets and deadends visited. Clearly, since each node has $4$ children and the output of the observator should be consistent, running it requires to build a complete tree, which we know to have the following number of nodes:
$$
n = \sum_{d=0}^{h-1}4^i,
$$
where $h$ is the height of the tree (i.e. our maximum depth). Just to make a quick example, if $h=7$ then $n=4^0+4^1+4^2+4^3+4^4+4^5+4^6=\num{5461}$. In this way, in the worst case, for each time-step we would have to examine $t\cdot n$ nodes, where $t$ is the number of trains, since one observation is needed for each agent. Following the previous example, in a scenario with $t=10$ agents and a tree observation with depth $h=7$, the reported features should be computed for each of the $\num{5641}\cdot 10=\num{56410}$ analyzed nodes at each time-step. 

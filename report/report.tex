...
We decided to perform the following rewards shaping:
\begin{itemize}
	\item If the agent has arrived to its target, a positive reward equal to the maximum number of steps minus the current number of steps is returned 
	\item If the agent is in deadlock, return a reward equal to the negated maximum number of steps 
	\item Accumulate rewards for choices if the agent is not in a decision cell

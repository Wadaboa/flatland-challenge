...
\begin{table}[h]
	...
	\begin{tabular}{@{}cc@{}}
		...
		\texttt{env.malfunctions.rate} &   $0.005$    \\
		\texttt{env.malfunctions.min\_duration} &   $15$    \\
		\texttt{env.malfunctions.max\_duration} &   $50$    \\
...
\section*{Binary tree with DQN}

Table \ref{table:bt-common-params} shows some hyperparameters that are common to all the experiments done with the binary tree observator and the DQN architecture.
...
\begin{table}[h]
	...
	\begin{tabular}{@{}cc@{}}
		...
		\bottomrule
	\end{tabular}
	\caption{Binary tree common hyperparameters}
	\label{table:bt-common-params}
\end{table}

\subsection*{Medium environment}

Table \ref{table:bt-medium-params} shows the hyperparameters that resulted in the best training convergence and test results on the medium environment, when all agents travel at the same speed and no malfunction occurs. In particular, testing results are shown in table \ref{bt-medium-res}. 
...
\begin{table}[h]
	...
	\begin{tabular}{@{}cc@{}}
		...
		\texttt{env.variable\_speed} &   False    \\
		\texttt{env.malfunctions.enabled} &   False    \\
		\texttt{env.rewards.stop\_penalty} &   $2.0$    \\
		\texttt{predictor.max\_depth}                 &   $7$   \\
		\texttt{observator.max\_depth}                  &   $7$    \\
		\texttt{observator.normalization\_radius}       &  $30$     \\ \bottomrule
	\end{tabular}
	\caption{Binary tree hyperparameters in medium environment}
	\label{table:bt-medium-params}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabular}{|c|c|c|c|}
	\hline
						   & \textbf{3 agents} & \textbf{5 agents} & \textbf{7 agents} \\ \hline
	\textbf{Fraction of dones}      &    $93.07\%$      &    $89.40\%$      &    $82.51\%$      \\ \hline
	\textbf{Fraction of deadlocks}  &    $0.0307$      &    $0.0596$      &    $0.1023$      \\ \hline
	\textbf{Mean normalized return} &    $-0.1404$      &     $-0.1581$     &    $-0.2057$      \\ \hline
	\textbf{Mean number of steps} &     $142$     &  $187$        &   $250$       \\ \hline
	\end{tabular}
	\caption{Binary tree results in medium environment}
	\label{table:bt-medium-res}
\end{table}

\subsection*{Big environment}
